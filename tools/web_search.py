...
_STRUCTURED_DESCRIPTION = (
    ...
    "注意：仅根据用户当前正在询问的内容决定是否搜索，不要因历史对话中出现过某个话题就无关联地去搜索。\n"
    "query 请用自然语言完整描述你想了解的问题。\n"
...
class WebSearchTool(BaseTool):
    ...
    async def execute(
        self,
        query: Annotated[str, "要搜索的内容，用自然语言完整描述你想了解的问题，不要拆成零散关键词"],
...
class DeepSeekWebSearchTool(BaseTool):
    ...
    async def execute(
        self,
        query: Annotated[str, "要搜索的问题，直接用自然语言完整描述，就像向一个人提问一样"],
    ) -> tuple[bool, str | dict[str, Any]]:
        """
        执行 DeepSeek 服务端联网搜索。
